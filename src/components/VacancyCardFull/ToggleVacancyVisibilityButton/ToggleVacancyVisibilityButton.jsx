...
export const ToggleVacancyVisibilityButton = ({ id }) => {
	...
	return (
		<button className={styles.btn} onClick={() => handleToggleVacancyVisibility(id)}>
			{
				isVacancyHidden ? (
					<>
						<Icon name="eye-solid" className={styles.icon} />
						<span>Показать</span>
					</>
				) : (
					<>
						<Icon name="eye-slash-solid" className={styles.icon} />
						<span>Скрыть</span>
					</>
				)
			}
